...
class RegisterCommand(commands.Cog):
    ...
    @commands.slash_command(name="register", description="Register your account with the bot")
    async def register(self, inter: AppCmdInter) -> None:
        api = AsyncAPIClient.instance()
        if api is None:
            await inter.response.send_message("API client not initialized. Please try again later.", ephemeral=True)
            return
        ...
        result = await api.sync_user(
            ...
            avatar=str(
                inter.author.avatar.url) if inter.author.avatar else None,
            banner=str(
                inter.author.banner.url) if inter.author.banner else None,
        )
